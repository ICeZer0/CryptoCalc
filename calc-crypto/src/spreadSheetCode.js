/*
 * Script by mix1009 (2017)
 * https://steemit.com/@mix1009
 */
...
function UPDATE() {
  ...
  range = sheet.getRange(3, 1, 100, 1);
  coins = range.getDisplayValues();

  range2 = sheet.getRange(2, 5, 1, 1); // get secondary currency.
  currency2 = range2.getDisplayValue();
  //Logger.log(currency2);
  ...
  outRange = sheet.getRange(3+parseInt(lastIdx)+2, 2, 1, 1);
...
function getCoinMarketCapPrice(coin, convertCurrency)
{
  ...
  change_1h = parseFloat(json[0]['percent_change_1h'])/100.0;
  change_1d = parseFloat(json[0]['percent_change_24h'])/100.0;
  change_7d = parseFloat(json[0]['percent_change_7d'])/100.0;
  rank = parseInt(json[0]['rank']);
  
  return [symbol, btc, usd, krw, change_1d, change_7d, rank]
}
...
function getDateTimeString(timestamp)
{
  ...
  if (0) { // Korean format
    ...
    var formattedTime = month + "월 " + day + "일" + " " + hours + ':' + minutes.substr(-2) + ':' + seconds.substr(-2);
...
function setupHistoryHeader(hsheet)
{
...

  {
    ...
    for (var idx in coins) {
      ...
      //Logger.log(coinSymbol + ":" + a1n + " = " + range2.getDisplayValue());
      //current[headerIdx] = range2.getDisplayValue();
